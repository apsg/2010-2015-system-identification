function [th, eo, ep] = kalman_2s_v2(fi, y, sw, sv)
warning off
W = diag([sw^2, sw^2]);
...
%  Starting conditions ("big" covariance matrix)
Pm(:,:,1) = diag([10,10]);

% Forward time filtering
...
for t=1:N
    ...
    % Holey smoother - first way
    Po(:,:,t) = inv(inv(Pmp(:,:,t)) + inv(Ppp(:,:,t)));
    tho(:,t) = Po(:,:,t) * (Pmp(:,:,t)\thmp(:,t) + Ppp(:,:,t)\thpp(:,t));
    eo(t) = y(t) - fi(:,t)' * tho(:,t);
    
    % corresponding holey smoother matching errors - quicker way
    q = fi(:,t)'*P(:,:,t)*fi(:,t);
    eo2(t) = e(t) / (1 - q);
    % corresponding patched smoother matching errors
    ep(t) = e(t) *(1+q);
end
warning on